...
# バーチャートレースを作成:(y軸可変)
anim <- ggplot(rank_df, aes(x = ranking, y = age, fill = groupID, color = groupID)) + 
  geom_bar(stat = "identity", width = 0.9, alpha = 0.8) + # 年齢バー
  geom_text(aes(label = paste(" ", age, "歳")), hjust = 0) + # 年齢ラベル
...
# gif画像を保存
gganimate::anim_save(filename = "BarChartRace/output/YearsAge_MinMax.gif", animation = g)
...
m <- gganimate::animate(
  ...
  nframes = n*(t+s), fps = (t+s)*mps, 
  width = 900, height = 600, 
  renderer = gganimate::av_renderer(file = "BarChartRace/output/YearsAge_MinMax.mp4")
)
