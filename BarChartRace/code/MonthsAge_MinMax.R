...
# gif画像を保存
gganimate::anim_save(filename = "BarChartRace/output/MonthsAge_MinMax.gif", animation = g)
...
m <- gganimate::animate(
  ...
  nframes = n*(t+s), fps = (t+s)*mps, 
  width = 900, height = 600, 
  renderer = gganimate::av_renderer(file = "BarChartRace/output/MonthsAge_MinMax.mp4")
)
...
# 画像を保存
ggplot2::ggsave(
  filename = paste0("BarChartRace/output/MonthsAge_MinMax_", date_val, ".png"), plot = graph, 
