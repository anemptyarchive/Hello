...
release_n_df <- single_df %>% 
  ...
  dplyr::mutate(
    ...
    artist_name = artistName %>% # グラフ表示名を追加
      ...
      stringr::str_replace(pattern = "ミニモニ。と高橋愛＋4KIDS", replacement = "ミニモニ。") %>% # 不要な文字列を削除
      ...
      stringr::str_replace(pattern = "ハロプロ研修生北海道 feat.稲場愛香", replacement = "ハロプロ研修生北海道"), # 不要な文字列を削除
    artist_idname = artist_name %>% # id割り当て用に編集
      ...
      stringr::str_replace(pattern = "S/mileage", replacement = "スマイレージ") %>%  # 改名前に統一
