...
　`summarise()`で、各メンバーの年齢を合計します。また、メンバー数をカウントします。ただし、誕生日情報がないメンバーがいるので、`is.na()`の結果を`!`で反転させて、欠損値`NA`でない(誕生日情報がある)メンバー数をカウントします。\
　合計年齢(`age_total`列)をメンバー数(`member_n`列)で割って、平均年齢(`age_mean`列)とします。\
