...
anim <- ggplot(
  ...
  geom_bar(
    mapping = aes(y = mean_age, fill = factor(groupID), color = factor(groupID)), 
...
m <- gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "ChartRace/output/YearsAge/YearsAge_mean.mp4")
)
