...
anim <- ggplot(
  ...
  geom_bar(
    mapping = aes(y = mean_period, fill = factor(groupID), color = factor(groupID)), 
  ...
  labs(
    title = "ハロプログループ・ユニットの平均活動月数の推移:(デビュー日基準)", 
    subtitle = paste0(
...
m <- gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "ChartRace/output/PeriodDebut/PeriodDebut_mean.mp4")
)
...
# 年数の最大値を取得
years_max <- max(rank_month_df[["mean_period"]], na.rm = TRUE) %/% 12
...
graph <- ggplot(
  ...
  labs(
    title = "ハロプログループ・ユニットの平均活動月数:(デビュー日基準)", 
...
# 画像を書出
ggplot2::ggsave(
  filename = paste0("ChartRace/output/PeriodDebut_mean_", date_val, ".png"), plot = graph, 
