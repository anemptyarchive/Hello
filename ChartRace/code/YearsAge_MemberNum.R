...
count_df <- join_df |> # メンバーID, 加入日, 卒業日
  ...
  dplyr::reframe(
    ...
    .by = c(memberID, memberName, birthDate)
  ) |> 
  dplyr::mutate(
  ...
  dplyr::summarise(
    member_num = dplyr::n(), # 年齢ごとのメンバー数
    .by = c(date, member_age)
  ) |> 
  dplyr::arrange(date, member_age)
count_df


### 演出用データの作成 -----

# 年齢構成が変化する前後月のデータを作成:(バーの変化の強調用)
outside_df <- count_df |> 
  tidyr::complete(
    date = seq(
      from = count_df |> 
...
label_df <- rank_df |> 
  ...
  dplyr::summarise(
    ...
    max_age   = max(tmp_age, na.rm = TRUE), # 最年長
    min_age   = min(tmp_age, na.rm = TRUE), # 最年少
    .by = date
  ) |> # (疑似データを除いて計算)
  dplyr::mutate(
    max_num = (max_num == 0) |> 
    ...
    mean_age  = (total_num > 0) |> 
      dplyr::if_else(
        true  = total_age / total_num, # グループ平均年齢
        false = 0, # 全て疑似データの場合
      ), 
    max_age = (max_age == -Inf) |> 
      dplyr::if_else(
    ...
    mean_label = paste(
      ...
      "平均年齢:", round(mean_age, digits = 1), "歳<br>", 
      "最年長:", "<span style='color:#ffffff;'>...</span>", max_age, "歳<br>", # (白文字による文字揃え)
      "最年少:", "<span style='color:#ffffff;'>...</span>", min_age, "歳"
    )
  ) |> 
  dplyr::select(date, max_num, mean_label)
label_df
...
anim <- ggplot() + 
  ...
  ggtext::geom_textbox(
    data    = label_df, 
    mapping = aes(x = max_rank+0.5, y = max_num, label = mean_label), 
  ...
  labs(
    title    = paste0(group_name, "：メンバーの年齢構成の推移"), 
    subtitle = paste(
...
# 動画を作成
file_path <- paste0("ChartRace/output/YearsAge_MemberNum/YearsAge_", agcy_flag, group_id, ".mp4")
m <- gganimate::animate(
  plot = anim, 
...
label_df <- hist_df |> 
  ...
  dplyr::select(date, mean_age, mean_label)
label_df
...
anim <- ggplot() + 
  ...
  ggtext::geom_textbox(
    data    = label_df, 
    mapping = aes(x = max_age+0.5, y = max_num, label = mean_label), 
  ...
  theme(
    ...
    plot.title    = element_text(color = "black", face = "bold", size = 20, hjust = 0.5), # 図タイトル
    plot.subtitle = element_text(color = "black", size = 15, hjust = 0.5), # 図サブタイトル
    plot.margin   = margin(t = 10, r = 30, b = 10, l = 60, unit = "pt"), # 図の余白
    legend.position = "none" # 凡例の位置
  ) + 
  labs(
    title    = paste0(group_name, "：メンバーの年齢構成の推移"), 
...
# 動画を作成
file_path <- paste0("ChartRace/output/YearsAge_MemberNum/YearsAge_hist_", agcy_flag, group_id, ".mp4")
m <- gganimate::animate(
  plot = anim, 
  nframes = (t+s)*n, fps = (t+s)*mps, 
  width = 900, height = 600, 
