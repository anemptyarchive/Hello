...
count_df <- join_df |> # メンバーID, 加入日, 卒業日
  ...
  dplyr::reframe(
    ...
    .by = c(memberID, memberName, birth_year)
  ) |> 
  dplyr::summarise(
    member_num = dplyr::n(), # 誕生年ごとのメンバー数
...
# 誕生年構成が変化する前後月のデータを作成:(バーの変化の強調用)
outside_df <- count_df |> 
  tidyr::complete(
    date = seq(
      from = count_df |> 
...
label_df <- rank_df |> 
  ...
  dplyr::mutate(
    ...
    num_label = paste(
      ...
      "最年長:", "<span style='color:#ffffff;'>...</span>", min_year, "年<br>", # (白文字による文字揃え)
      "最年少:", "<span style='color:#ffffff;'>...</span>", max_year, "年"
    )
  ) |> 
...
anim <- ggplot() + 
  ...
  theme(
    ...
    plot.margin   = margin(t = 10, r = 60, b = 10, l = 90, unit = "pt"), # 図の余白
    legend.position = "none" # 凡例の位置
  ) + 
  labs(
    title    = paste0(group_name, "：メンバーの誕生年構成の推移"), 
    subtitle = paste(
...
# 動画を作成
file_path <- paste0("ChartRace/output/YearOfBirth_MemberNum/YearOfBirth_", agcy_flag, group_id, ".mp4")
m <- gganimate::animate(
  plot = anim, 
...
anim <- ggplot() + 
  ...
  theme(
    ...
    plot.title    = element_text(color = "black", face = "bold", size = 20, hjust = 0.5), # 図タイトル
    plot.subtitle = element_text(color = "black", size = 15, hjust = 0.5), # 図サブタイトル
    plot.margin   = margin(t = 10, r = 30, b = 10, l = 90, unit = "pt"), # 図の余白
    legend.position = "none" # 凡例の位置
  ) + 
  labs(
    title    = paste0(group_name, "：メンバーの誕生年構成の推移"), 
...
# 動画を作成
file_path <- paste0("ChartRace/output/YearOfBirth_MemberNum/YearOfBirth_hist_", agcy_flag, group_id, ".mp4")
m <- gganimate::animate(
  plot = anim, 
  nframes = (t+s)*n, fps = (t+s)*mps, 
  width = 900, height = 600, 
