...
# グループごとの新メンバー・古参メンバーハロプロデビュー月数(活動歴)の推移 -----
...
rank_df <- group_name_df |> # 活動月, グループ名, 結成(改名)月, 解散(改名)月
  ...
  dplyr::left_join(
    member_df |> # デビュー日
      dplyr::select(memberID, memberName, debutDate) |> 
      dplyr::slice_min(debutDate, n = 1, with_ties = FALSE, by = memberID), # 重複を除去
    by = "memberID"
  ) |> 
  dplyr::mutate(
    member_period = lubridate::interval(start = debutDate, end = date) |> 
...
anim <- ggplot(
  ...
  geom_text(
    mapping = aes(y = member_period, label = paste(" ", period_years, "年", round(period_months, digits = 1), "か月")), 
    hjust = 0
  ) + # 月数ラベル
  geom_text(
  ...
  labs(
    title = ifelse(
      ...
      yes  = "ハロプログループ・ユニットの最小活動月数の推移:(デビュー日基準)", 
      no   = "ハロプログループ・ユニットの最大活動月数の推移:(デビュー日基準)"
    ), 
    subtitle = paste0(
...
m <- gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = paste0("ChartRace/output/PeriodDebut/PeriodDebut_", MinMax_flag, ".mp4"))
)
...
# 年数の最大値を取得
years_max <- max(rank_month_df[["member_period"]], na.rm = TRUE) %/% 12
...
graph <- ggplot(
  ...
  labs(
    title = ifelse(
      test = MinMax_flag == "min", 
      yes  = "ハロプログループ・ユニットの最小活動月数:(デビュー日基準)", 
      no   = "ハロプログループ・ユニットの最大活動月数:(デビュー日基準)"
    ), 
...
# 画像を書出
ggplot2::ggsave(
  filename = paste0("ChartRace/output/PeriodDebut_", MinMax_flag, "_", date_val, ".png"), plot = graph, 
