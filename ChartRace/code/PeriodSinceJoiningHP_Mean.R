...
anim <- ggplot(
  ...
  labs(
    title = "ハロプログループ・ユニットの平均活動月数の推移:(ハロプロ加入日基準)", 
    subtitle = paste0(
...
m <- gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = "ChartRace/output/PeriodJoinHP/PeriodJoinHP_mean.mp4")
)
...
graph <- ggplot(
  ...
  labs(
    title = "ハロプログループ・ユニットの平均活動月数:(ハロプロ加入日基準)", 
