...
rank_df <- group_name_df |> # 活動月, グループ名, 結成(改名)月, 解散(改名)月
  ...
  dplyr::select(date, groupID, groupName, member_age, ranking)
rank_df
...
anim <- ggplot(
  ...
  geom_bar(
    mapping = aes(y = member_age, fill = factor(groupID), color = factor(groupID)), 
...
m <- gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = paste0("ChartRace/output/YearsAge/YearsAge_", MinMax_flag, ".mp4"))
)
...
rank_month_df <- group_df |> # グループ名, 結成(改名)月, 解散(改名)月
  ...
  dplyr::select(date, groupID, groupName, member_num, member_age, ranking) |> 
...
graph <- ggplot(
  ...
  geom_text(
    mapping = aes(y = 0, label = paste0("  ", member_age, "歳 (", member_num, "人)")), 
    hjust = 0, color = "white"
  ) + # 年齢ラベル
  geom_text(
    mapping = aes(y = 0, label = paste(groupName, " ")), 
...
# 画像を書出
ggplot2::ggsave(
  filename = paste0("ChartRace/output/YearsAge_", MinMax_flag, "_", date_val, ".png"), plot = graph, 
