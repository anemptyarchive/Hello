...
rank_df <- group_name_df |> # 活動月, グループ名, 結成(改名)月, 解散(改名)月
  ...
  dplyr::summarise(
    member_exp = dplyr::if_else(
      MinMax_flag == "min", 
      true  = min(member_exp, na.rm = TRUE), # 最小加入月数
      false = max(member_exp, na.rm = TRUE), # 最大加入月数
    ), 
  ...
  dplyr::arrange(date, member_exp, groupID) |> # 順位付け用
  dplyr::mutate(
    exp_years  = member_exp %/% 12, # グループ平均加入年数
    exp_months = member_exp %% 12,  # グループ平均加入月数 - 平均加入年数
    ranking    = dplyr::row_number(-member_exp), # 順位
    .by = date
  ) |> 
  dplyr::select(date, groupID, groupName, member_exp, exp_years, exp_months, ranking) |> 
...
anim <- ggplot(
  ...
  geom_bar(
    mapping = aes(y = member_exp), 
    stat = "identity", width = 0.9, alpha = 0.8
  ) + # 月数バー
  geom_text(
    mapping = aes(y = member_exp, label = paste(" ", exp_years, "年", round(exp_months, digits = 1), "か月")), 
    hjust = 0
  ) + # 月数ラベル
  geom_text(
...
m <- gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = paste0("ChartRace/output/JoinExp_", MinMax_flag, ".mp4"))
)
...
rank_month_df <- group_df |> # グループ名, 結成(改名)月, 解散(改名)月
  ...
  dplyr::select(date, groupID, groupName, member_num, member_exp, exp_years, exp_months, ranking) |> 
...
# 年齢の最大値を取得
years_max <- max(rank_month_df[["member_exp"]]) %/% 12
...
graph <- ggplot(
  ...
  geom_text(
    mapping = aes(
      y = 0, 
      label = paste0("  ", exp_years, "歳", exp_months, "か月", " (", member_num, "人)")
    ), 
    hjust = 0, color = "white"
  ) + # 月数ラベル
  geom_text(
...
# 画像を書出
ggplot2::ggsave(
  filename = paste0("ChartRace/output/JoinExp_", MinMax_flag, "_", date_val, ".png"), plot = graph, 
