...
# グループごとの新メンバー・古参メンバーグループ加入月数(活動歴)の推移 ---------
...
anim <- ggplot(
  ...
  labs(
    title = ifelse(
      ...
      yes  = "ハロプログループ・ユニットの最小活動月数の推移:(グループ加入日基準)", 
      no   = "ハロプログループ・ユニットの最大活動月数の推移:(グループ加入日基準)"
    ), 
    subtitle = paste0(
...
m <- gganimate::animate(
  ...
  renderer = gganimate::av_renderer(file = paste0("ChartRace/output/PeriodJoinGroup/PeriodJoinGroup_", MinMax_flag, ".mp4"))
)
...
graph <- ggplot(
  ...
  labs(
    title = ifelse(
      test = MinMax_flag == "min", 
      yes  = "ハロプログループ・ユニットの最小活動月数:(グループ加入日基準)", 
      no   = "ハロプログループ・ユニットの最大活動月数:(グループ加入日基準)"
    ), 
...
# 画像を書出
ggplot2::ggsave(
  filename = paste0("ChartRace/output/PeriodJoinGroup_", MinMax_flag, "_", date_val, ".png"), plot = graph, 
